...
// Detected 802.11 PHY generation. Mapped from which capability IEs the AP advertises.
enum class WifiStandard { Unknown, A, B, G, N, AC, AX, BE };
...
struct Network
{
	...
	// Center frequency of the primary 80/160 MHz segment in MHz, derived from the
	// VHT/HE Operation IE (CCFS0). 0 when the AP is 20/40 MHz only or unknown.
	...
	// Theoretical max PHY rate in Mbps for a single link (peak MCS × streams × width).
	// 0 when we lack enough information to estimate.
	uint32_t _maxRateMbps {0};

	// BSS Load IE — number of associated stations. -1 means the AP didn't advertise it.
	int16_t _stationCount {-1};

	// BSS Load IE — channel utilisation byte (0-255, 255 = fully busy). -1 if absent.
	int16_t _channelUtilization {-1};

	// Number of spatial streams (1-8) inferred from the HT/VHT MCS bitmaps. 0 = unknown.
