...
// Graphical front-end (wifiscan --gui). Mirrors ui::App: owns the background scan
// thread + shared state and runs the render loop on the main thread. Uses a GLFW +
// OpenGL3 Dear ImGui backend instead of FTXUI.
class App
{
public:
	explicit App(wifi::IScanner& scanner);

	// Opens the window and runs the ImGui loop; blocks until the window is closed
	// or the user presses 'q' / Escape.
	void Run();
...
private:
	...
	// Background thread body: mirrors ui::App::ScanLoop but wakes the render loop
	// via glfwPostEmptyEvent() (thread-safe) instead of FTXUI's task queue.
	...
	// Declared last so it is destroyed first: its destructor request_stop()+join()s
	// before the shared state it touches is torn down (same invariant as ui::App).
