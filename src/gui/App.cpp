...
App::App(wifi::IScanner& scanner)
	: _scanner(scanner), _statusBarPanel(scanner)
{
}
...
void App::Run()
{
	...
	_window = glfwCreateWindow(static_cast<int>(1100 * dpiScale),
							   static_cast<int>(720 * dpiScale), "wifiscan",
							   nullptr, nullptr);
	if (_window == nullptr)
	{
		fprintf(stderr,
	...
	{
		// Idle-efficient: woken by input or by the scan thread's postEmptyEvent.
	...
	// Stop the scan thread BEFORE tearing down GLFW: a late glfwPostEmptyEvent()
	// after glfwTerminate() would crash.
