...
void SortByMode(std::vector<wifi::Network>& nets, SortMode mode)
{
	switch (mode)
	{
		case SortMode::Signal:
			std::ranges::sort(nets, [](const auto& a, const auto& b) { return a._signalDbm > b._signalDbm; });
			break;
		case SortMode::SSID:
			std::ranges::sort(nets, [](const auto& a, const auto& b) { return a._ssid < b._ssid; });
			break;
		case SortMode::Channel:
			std::ranges::sort(nets, [](const auto& a, const auto& b) { return a._channel < b._channel; });
			break;
	}
}
...
ftxui::Element BuildHeaderRow(SortMode active)
{
	...
	auto sep = [] { return text(" | ") | color(theme::Color(theme::UiColor::Muted)); };
	...
	{
		return active == mode ? theme::Color(theme::UiColor::DataValue) : theme::Color(theme::UiColor::ColumnHeader);
	};
...
ftxui::Element BuildQualityBar(int signalDbm, int quality)
{
	...
	return hbox({
		text(label.substr(0, static_cast<size_t>(filled)))
			| color(Color::Black)
			| bgcolor(theme::SignalColor(signalDbm)),
		text(label.substr(static_cast<size_t>(filled)))
			| color(theme::SignalColor(signalDbm))
...
ftxui::Element BuildDataRow(const wifi::Network& net)
{
	...
	auto prefixColor = net._connected ? theme::Color(theme::UiColor::ConnectedNetwork) : theme::Color(theme::UiColor::Muted);
	...
	return hbox({
		...
		sep(),
		text(PadRight(net._bssid, 17)) | color(theme::Color(theme::UiColor::Muted)),
		sep(),
